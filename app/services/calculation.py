...
from app.services.unit_converter import convert_unit, load_custom_conversions, normalize_to_base
...
def calculate_shopping_list(db: Session, camp_id: int) -> dict[str, Any]:
    ...
    for meal_plan in meal_plans:
        ...
        for ingredient_data in scaled["ingredients"]:
            ...
            quantity, unit = normalize_to_base(ingredient_data["quantity"], ingredient_data["unit"])

            key = ((ingredient.name or "").strip().casefold(), (unit or "").strip().casefold())
    ...
    # Load user-defined unit conversions (e.g. Becher -> ml) so the shopping
    # list displays quantities in the configured target unit, not the raw input.
    ...
    for data in aggregated.values():
        converted = convert_unit(data["quantity"], data["unit"], custom_conversions)

        camp_note = _merge_notes(camp_notes.get(iid) for iid in data["ingredient_ids"])
        global_note = _merge_notes(data["global_notes"])

        category = data["ingredient"].category
        category_name = category.name if category else "Sonstiges"
        category_colors.setdefault(category_name, category.color if category else "#6B7280")
    ...
    for category in sorted(categories.keys()):
        sorted_categories[category] = sorted(categories[category], key=lambda x: x["ingredient"].name)
...
def get_camp_statistics(db: Session, camp_id: int) -> dict[str, Any]:
    ...
    if recipe_ids:
        ...
        recipes_to_check = (
            ...
            .options(selectinload(models.Recipe.allergens))
            .all()
        )
    ...
    for day_num in range(total_days):
        ...
        daily_overview.append(
            {"day_number": day_num + 1, "date": day_date, "meals": day_meals, "meals_planned": meals_planned}
        )
