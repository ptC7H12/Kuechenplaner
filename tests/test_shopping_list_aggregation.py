...
from app.services.unit_converter import add_custom_conversion
...
def _make_ingredient(
    db_session, name: str, *, unit: str = "kg", category: str = "Fleisch", note: str | None = None
) -> models.Ingredient:
    db_category = crud.get_or_create_category(db_session, category, commit=False)
    ing = models.Ingredient(name=name, unit=unit, category_id=db_category.id, note=note)
    db_session.add(ing)
    db_session.flush()
    return ing


def _make_recipe_with(
    db_session,
    ingredient: models.Ingredient,
    quantity: float,
    unit: str,
    recipe_name: str = "Spaghetti Bolognese",
    base_servings: int = 10,
) -> models.Recipe:
...
def test_duplicate_ingredient_rows_collapse_into_one_line(db_session):
    ...
    recipe_a = _make_recipe_with(db_session, ing_a, quantity=1.0, unit="kg", recipe_name="Lasagne")
    recipe_b = _make_recipe_with(db_session, ing_b, quantity=3.3, unit="kg", recipe_name="Chili")
...
def test_mass_units_collapse_into_one_line(db_session):
    ...
    recipe_g = _make_recipe_with(db_session, ing, quantity=500, unit="g", recipe_name="Gratin")
    recipe_kg = _make_recipe_with(db_session, ing, quantity=2.0, unit="kg", recipe_name="Bolognese")
...
def test_volume_units_collapse_into_one_line(db_session):
    ...
    recipe_ml = _make_recipe_with(db_session, ing, quantity=800, unit="ml", recipe_name="Punsch")
    recipe_l = _make_recipe_with(db_session, ing, quantity=1.0, unit="L", recipe_name="Saftmix")
...
def test_mehl_g_kg_el_yields_two_lines(db_session):
    ...
    recipe_g = _make_recipe_with(db_session, ing, quantity=900, unit="g", recipe_name="Fladenbrot")
    recipe_kg = _make_recipe_with(db_session, ing, quantity=1.5, unit="kg", recipe_name="Pizzateig")
    recipe_el = _make_recipe_with(db_session, ing, quantity=8, unit="EL", recipe_name="Apple crumble")
    _add_meal_plan(db_session, camp.id, recipe_g.id, position=0)
    _add_meal_plan(db_session, camp.id, recipe_kg.id, position=1)
...
def test_single_unit_aggregation_unchanged(db_session):
    ...
    recipe_a = _make_recipe_with(db_session, ing, quantity=5, unit="Stück", recipe_name="Apfelsalat")
    recipe_b = _make_recipe_with(db_session, ing, quantity=3, unit="Stück", recipe_name="Apfelmus")
    _add_meal_plan(db_session, camp.id, recipe_a.id, position=0)
    _add_meal_plan(db_session, camp.id, recipe_b.id, position=1)
...
def test_different_units_do_not_collapse(db_session):
    ...
    recipe_a = _make_recipe_with(db_session, ing_a, quantity=10.0, unit="EL", recipe_name="Kuchen")
    recipe_b = _make_recipe_with(db_session, ing_b, quantity=200.0, unit="g", recipe_name="Pudding")
    _add_meal_plan(db_session, camp.id, recipe_a.id, position=0)
    _add_meal_plan(db_session, camp.id, recipe_b.id, position=1)
...
def test_custom_conversion_is_applied(db_session):
    """A user-defined conversion (Becher -> ml) configured in settings must be
    applied to the shopping list instead of leaving the raw input unit."""
    add_custom_conversion(db_session, from_unit="Becher", to_unit="ml", threshold=1, factor=250)

    camp = _make_camp(db_session)
    ing = _make_ingredient(db_session, "Sahne", unit="Becher", category="Milchprodukte")
    recipe = _make_recipe_with(db_session, ing, quantity=4, unit="Becher", recipe_name="Sahnesoße")
    _add_meal_plan(db_session, camp.id, recipe.id, position=0)

    result = calculate_shopping_list(db_session, camp.id)
    items = result["items"]
    assert len(items) == 1
    item = items[0]
    # 4 Becher * 250 = 1000 ml (target unit from the configured conversion).
    assert item["unit"] == "ml"
    assert item["quantity"] == pytest.approx(1000)
...
def test_notes_are_merged_across_duplicates(db_session):
    ...
    camp = _make_camp(db_session)
    ing_a = _make_ingredient(db_session, "Mehl", unit="g", category="Backwaren", note="Bio-Variante")
    ing_b = _make_ingredient(db_session, "mehl", unit="g", category="Backwaren", note="beim Großhändler bestellen")
    recipe_a = _make_recipe_with(db_session, ing_a, quantity=500, unit="g", recipe_name="Brot")
    recipe_b = _make_recipe_with(db_session, ing_b, quantity=300, unit="g", recipe_name="Kuchen")
    _add_meal_plan(db_session, camp.id, recipe_a.id, position=0)
    _add_meal_plan(db_session, camp.id, recipe_b.id, position=1)
...
def test_duplicate_notes_are_deduplicated(db_session):
    ...
    camp = _make_camp(db_session)
    ing_a = _make_ingredient(db_session, "Salz", unit="g", category="Gewürze", note="grobes Meersalz")
    ing_b = _make_ingredient(db_session, "salz ", unit="g", category="Gewürze", note="grobes Meersalz")
    recipe_a = _make_recipe_with(db_session, ing_a, quantity=20, unit="g", recipe_name="Suppe")
    recipe_b = _make_recipe_with(db_session, ing_b, quantity=30, unit="g", recipe_name="Eintopf")
    _add_meal_plan(db_session, camp.id, recipe_a.id, position=0)
    _add_meal_plan(db_session, camp.id, recipe_b.id, position=1)
